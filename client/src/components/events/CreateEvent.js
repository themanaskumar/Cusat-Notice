...
const CreateEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Submitting event data with', attachments.length, 'attachments');
      
      await axios.post('/api/events', formDataToSubmit, {
        headers: {
      ...
      setError(
        ...
        `Error ${err.response?.status || ''}: ${err.message || 'Failed to create event'}`
      );
    } finally {
      setLoading(false);
    }
